...
class LoginScreen(MDScreen):
    def __init__(self, **kwargs):
        ...
        box.add_widget(self.btn_login)

        self.btn_to_register = MDFlatButton(
            text=self._tr_or("switch_to_register", "Switch to Register"),
            size_hint_y=None,
            height=dp(48),
            on_release=lambda x: setattr(self.manager, "current", "register")
        )
        box.add_widget(self.btn_to_register)
    ...
    def change_language(self, *args):
        ...
        self.title.text = self._tr_or("login", "Login")
        self.username.hint_text = self._tr_or("username", "Username")
        self.password.hint_text = self._tr_or("password", "Password")
        self.btn_login.text = self._tr_or("login", "Login")
        self.btn_to_register.text = self._tr_or("switch_to_register", "Switch to Register")
